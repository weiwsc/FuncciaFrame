...
namespace vva::gfx::vulkan {
    struct TextureSamplerDescriptorSet {
        ...
        auto writeSamplers(std::span<const vk::raii::Sampler> samplers) const -> void;
        auto writeTexture(uint32_t slot, vk::ImageView view) const -> void;
        auto writeTexture(uint32_t slot, std::span<const vk::ImageView> views) const -> void;
    };
}
