...
namespace Funccia::UI {
    ...
    auto UIElement::contentBoxOnAxis(Axis axis) const -> float {
        switch (axis) {
            case Axis::Horizontal:
                return m_border_box_size.GetX() - m_style.padding.left() - m_style.padding.right() - m_style.border_widths.left() - m_style.border_widths.right();
            case Axis::Vertical:
                return m_border_box_size.GetY() - m_style.padding.top() - m_style.padding.bottom() - m_style.border_widths.top() - m_style.border_widths.bottom();
        }
    }
    ...
    auto UIElement::CullingPass(vec4 parent_clipping_box) -> void {
        ...
        content_culling_box.z -= m_style.border_widths.right();   //without cutting the border width, the clipped child will overlap the parent's border. if the border box calculated is changed, PLS CHANGE THIS AS WELL!!!!!!
    ...
    auto UIElement::RenderQueue(UIRender &render, Graphic::GL::TextRenderer &text_renderer,float parent_content_box_x, float parent_content_box_y) -> void {
        ...
        // };
        vec4 borderColor;
        if (m_style.border_color.r == 0 && m_style.border_color.g == 0 && m_style.border_color.b == 0 && m_style.border_color.a == 0) {
        ...
        render.Collect({
            ...
            m_style.border_radius.r, m_style.border_radius.g, m_style.border_radius.b, m_style.border_radius.a,
            // a_shadowProperties (4 floats)
            m_style.shadow.shadow_offset.x, m_style.shadow.shadow_offset.y, m_style.shadow.blur_radius, m_style.shadow.spread,
            // a_shadowColor (4 floats)
            m_style.shadow.shadow_color.r, m_style.shadow.shadow_color.g, m_style.shadow.shadow_color.b, m_style.shadow.shadow_color.a,
            // a_borderWidths (4 floats)
            m_style.border_widths.top(), m_style.border_widths.right(), m_style.border_widths.bottom(), m_style.border_widths.left(),
