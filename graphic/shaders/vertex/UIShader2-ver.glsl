...
layout(location=6) in vec4 a_borderWidths;  // (T, R, B, L)  ← note order
...
void main() {
    // conservative cover (for shadow/blur), same as your logic
    vec2 ofs    = a_shadowProps.xy;
    float blur  = max(a_shadowProps.z, 0.0);
    float spread= a_shadowProps.w;

    // use max border thickness to avoid clipping AA at edges
